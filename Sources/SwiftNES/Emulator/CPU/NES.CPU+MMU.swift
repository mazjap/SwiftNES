    ...
    public class MemoryManagementUnit: Memory {
        ...
        public func read(from address: UInt16) -> UInt8 {
            ...
            case 0x0000...0x1FFF:
                ...
                return internalRAM.read(from: MemoryMap.resolveRamAddress(address: address))
            case 0x2000...0x3FFF:
                guard let readPPURegister else {
                ...
                return readPPURegister(resolvedAddress)
            case 0x4000...0x4017:
                // TODO: - Retreive from APU and IO registers
                return 0; #warning("TODO: - Retreive from APU and IO registers")
            case 0x4018...0x401F:
        ...
        public func write(_ value: UInt8, to address: UInt16) {
            ...
            case 0x0000...0x1FFF:
                // Internal RAM and its mirrors
                internalRAM.write(value, to: MemoryMap.resolveRamAddress(address: address))
            case 0x2000...0x3FFF:
                guard let writePPURegister else {
                    emuLogger.error("Received access/modify request to PPU register, but no PPU register access handlers were provided")
                    return
                }
                ...
                writePPURegister(value, resolvedAddress)
            case 0x4014:
                handleOAMDMA?(value)
            case 0x4000...0x4017:
                // TODO: - Retreive from APU and IO registers
